...
```
  ...
  There are some "unknown" in column "mating" and "denning", and there is no other NA's in other variables
...
polar_bears %>% 
  ...
  labs(x="Sex",y="Number of Bears Sampled",title="Number of males and females sampled by population")
```

  For both population, there are more female sampled. It is almost even for CS population, but not even sampling for SB population.
...
```{r}
polar_bears <- polar_bears %>% 
  mutate(capture_year=year(ymd(capture_date))) %>% 
  mutate(capture_year=as.numeric(capture_year))

polar_bears %>% 
  group_by(capture_year,popn) %>% 
  count() %>% 
  ggplot(aes(x=capture_year,y=n,color=popn))+
  geom_line()+
  geom_point()+
  scale_x_continuous(breaks=seq(1980,2020,by=5))+
  labs(x="Year",y="Number of Bears Sampled",title="Bears sampled by year for both population")
...
```{r}
polar_bears %>% 
  summarise(min_urea=min(urea_nitrogen),median_urea=median(urea_nitrogen),mean_urea=mean(urea_nitrogen),max_urea=max(urea_nitrogen))
```


```{r}
polar_bears %>% 
  summarise(min_creatinine=min(creatinine),median_creatinine=median(creatinine),mean_creatinine=mean(creatinine),max_creatinine=max(creatinine))
```
...
polar_bears %>% 
  select(bear_id,urea_nitrogen,creatinine,un_creat_ratio)
```
...
polar_bears %>% 
  select(bear_id,un_creat_ratio,fasting)
```
...
polar_bears %>% 
  ...
  geom_point()+
  scale_x_continuous(breaks=seq(1980,2020,by=5))+
  labs(x="Year",y="Number of Fasting Bears",title="Number of Fasting Bears by year for both population")
```
...
```{r}
lon_min=min(polar_bears$long,na.rm=T)
lon_max=max(polar_bears$long,na.rm=T)
lat_min=min(polar_bears$lat,na.rm=T)
lat_max=max(polar_bears$lat,na.rm=T)
...
ggmap(map1)+
  geom_point(data=polar_bears,mapping=aes(x=long,y=lat,color=popn),size=0.75,alpha=0.8)+
  labs(x="Longitude",y="Latitude",title="Polar Bears Locations")
```
...
server <- function(input, output, session) {
  output$plot = renderPlot({
    polar_bears %>% 
      ...
      labs(x="Population",y="urea nitrogen to creatinine ratio",title = "Range of urea nitrogen to creatinine ratio for both population")
  })
}
