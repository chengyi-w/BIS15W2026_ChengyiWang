...
ui <- fluidPage(
  selectInput("x","Select X Variable", choices = c("bill_length_mm","bill_depth_mm","flipper_length_mm", "body_mass_g"),selected = "bill_length_mm"),
  selectInput("y","Select Y Variable", choices = c("bill_length_mm","bill_depth_mm","flipper_length_mm", "body_mass_g"),selected = "body_mass_g")
)
...
ui <- fluidPage(
  selectInput("x","Select X Variable", choices = c("bill_length_mm","bill_depth_mm","flipper_length_mm", "body_mass_g"),selected = "bill_length_mm"),
  selectInput("y","Select Y Variable", choices = c("bill_length_mm","bill_depth_mm","flipper_length_mm", "body_mass_g"),selected = "body_mass_g"),
  checkboxInput("show_lm","Show best-fit line",value=F),
  plotOutput("plot",width = "500px", height = "400px")
)
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    ggplot(data=penguins,
           aes(x=.data[[input$x]],y=.data[[input$y]],color=species))+
      geom_point(na.rm=T)+
      if (input$show_lm)
        geom_smooth(method="lm",se=F,na.mr=T)
      else
        NULL
  })
}
...
```{r, eval=F, echo=T}
ui <- fluidPage(
  selectInput("fill","Select Fill Variable", choices = c("trophic_guild","thermoregulation"),selected = "trophic_guild"),
  plotOutput("plot",width = "500px", height = "400px")
)
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    ggplot(data=homerange,
           aes(x=locomotion,fill=.data[[input$fill]]))+
    geom_bar(na.rm=T)
  })
}
