...
```{r}
elephants %>% 
  summarize(across(c("estimated_age_years","shoulder_height_in_cm","tusk_length_in_cm","tusk_circumference_in_cm"),~mean(.,na.rm=T)))
```
...
elephants %>% 
  ...
  summarise(mean_estimated_age_years=mean(estimated_age_years,na.rm=T)
            ,n=n())
...
elephants %>% 
  ...
  group_by(sex) %>% 
  summarize(mean_age=mean(estimated_age_years, na.rm=T),median_age=median(estimated_age_years, na.rm=T),sd_age=sd(estimated_age_years, na.rm=T),n=n())
```
...
msleep %>% 
  group_by(vore) %>% 
  summarise(min_weight=min(bodywt,na.rm=T),max_weight=max(bodywt,na.rm=T),mean_weight=mean(bodywt,na.rm=T),n=n())
```
