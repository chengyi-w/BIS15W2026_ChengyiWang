...
gap %>% 
  ...
  labs(title="GDP per Capita in USA, 2000 - 2025",x="Year",y="GDP")
```
...
gap %>% 
  filter(year==1935) %>% 
  ggplot(aes(x=lex))+
  geom_density(color="black",fill="turquoise4",alpha=0.5)+
  labs(title="Distribution of Life Expectancy")
```
...
gap %>% 
  ...
  labs(title="Distribution of Life Expectancy of 1930 and 1950")  
```
...
income_cat2000 <- gap %>% 
  ...
  mutate(income_category=case_when(gdp<=1100 ~ "low",
                                   ...
                                   gdp>13900 ~"high")) %>% 
  mutate(income_category=factor(income_category,levels = c("low",
                                                           "low_middle",
                                                           "upper_middle",
                                                           "high"))) %>% 
  na.omit()
