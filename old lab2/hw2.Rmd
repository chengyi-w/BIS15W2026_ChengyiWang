...
mountain_name <- mountains_data$mountain_name
mountain_height_feet <- mountains_data$height_feet
mountain_height_meters <- mountains_data$height_meters
```

**5. Combine these vectors into a data frame called `mountains`.**
```{r}
tibble(mountain_name,mountain_height_feet,mountain_height_meters)
```

**6. What is the mean height of the mountains in feet?**
```{r}
mean(mountain_height_feet)
```
...
```{r}
first_climbed <- mountains_data$first_climbed
```
