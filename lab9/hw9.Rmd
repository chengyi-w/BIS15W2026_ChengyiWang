...
```{r}
sydneybeaches_mean_enterococci <- sydneybeaches_long %>% 
  group_by(year,site) %>% 
...
```{r}
sydneybeaches_mean_enterococci %>% 
  pivot_wider(names_from = "year", values_from = "mean_enterococci")
```
...
Overall, which year has the most dirty water
```{r}
sydneybeaches_mean_enterococci %>% 
  group_by(year) %>% 
  summarise(total_mean_enterococci=sum(mean_enterococci)) %>% 
  arrange(desc(total_mean_enterococci))
```
