...
The columns are all just range of income, which are should be one variable, income.
...
```{r}
billboard %>% 
  pivot_longer(-c(artist,track,date.entered),names_to = "week",values_to = "rank", values_drop_na = T)
```
...
qpcr_tidy %>% 
  ...
  separate(rep, into=c("rep","r_number"),sep=3)
```
