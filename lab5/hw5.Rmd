...
```{r}
ggplot(homerange, mapping=aes(x=taxon))+
  geom_bar()+
  labs(x="Animal Groups",title="Number of Animals in Each Group")
...
```{r}
ggplot(homerange, mapping=aes(x=trophic.guild ))+
  geom_bar()+
  labs(x="Trophic Guild",title="Number in Each Trophic Guild")
```
...
```{r}
homerange %>% 
  select(common.name, species, mean.hra.m2) %>% 
...
```{r}
homerange %>% 
  select(trophic.guild ,genus, species, mean.hra.m2) %>% 
...
homerange %>% 
  ggplot(mapping=aes(x=log10.mass,y=log10.hra))+
    ...
    labs(x="Logarithmic Scale Body Weight",y="Logarithmic Scale Homerange Size", title="Body Mass vs Homerange Size")
```
...
homerange %>% 
  ...
  ggplot(mapping = aes(x=class,y=log10.hra))+
  geom_boxplot()+
  labs(y="Logarithmic Scale Homerange Size",title="Homerange Sizes of Birds and Mammals")
...
Question: Among birds, which species have the smallest mean home range areas?
```{r}
homerange %>%
  filter(class == "aves", !is.na(mean.hra.m2)) %>%
  select(common.name, genus, species, mean.hra.m2) %>%
  arrange(mean.hra.m2)
```
