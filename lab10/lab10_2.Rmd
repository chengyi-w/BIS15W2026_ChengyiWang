...
```{r}
p+
  geom_point(aes(color=thermoregulation))
```

We can also map shapes to another categorical variable.
```{r}
p+
  geom_point(aes(color=thermoregulation,shape=thermoregulation))
```

What about just filling in with a color preference or size?
```{r}
p+geom_point(size=3,color="black",alpha=0.6,shape=24,fill="whitesmoke")
```
...
```{r}
homerange %>% 
  ggplot(aes(y=log10_mass,x=class,group = taxon))+
  geom_boxplot()
```
...
```{r}
homerange %>% 
  ggplot(aes(y=log10_mass,x=class,fill= taxon))+
  geom_boxplot()
```
