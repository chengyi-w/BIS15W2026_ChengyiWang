...
```{bash}
mv UKWFY9VP014-Alignment-Descriptions.csv BIS15W2026_ChengyiWang/lab16/data/blast_results/genome01_blastn.csv
```
...
```{r}
genome01_blastn <- read_csv("data/blast_results/genome01_blastn.csv") %>% clean_names()
```
...
```{bash}
mv UKXTYDAD016-Alignment-Descriptions.csv BIS15W2026_ChengyiWang/lab16/data/blast_results/aa_blastp_results.csv
```

2. Read the csv file into R and clean the column names. Save the cleaned data frame as `blastp_results`.

```{r}
blastp_results <- read_csv("data/blast_results/aa_blastp_results.csv") %>% clean_names()
```
